...
@app.route("/api/workshops/<int:workshop_id>/employees", methods=["GET"])
@jwt_required()
...
@app.route("/api/workshops/<int:workshop_id>/employees", methods=["POST"])
@jwt_required()
def create_employee(workshop_id):
    ...
    allowed_roles = ["gerente", "coordinador", "mecanico", "administrador"]

    if role not in allowed_roles:
        return jsonify({
            "message": "Role must be gerente, coordinador, mecanico or administrador"
        }), 400
...
@app.route("/api/workshops/<int:workshop_id>/employees/<int:employee_id>", methods=["GET"])
@jwt_required()
